...
containerDiv.classList.add("container" ,"w-50");
...
paginationDiv.classList.add('pagination','m-2')

containerDiv.append(tableDiv,paginationDiv);
document.body.append(containerDiv);


let currentPageId=1
getDataByPageNo(currentPageId)

function getDataByPageNo(pageNo)
{
  currentPageId=pageNo
  showData(data,pageNo)
  showPagination(data)
}

function showData(data,pageNo)
{
    console.log(pageNo)
    ...
    var th1 = createNewElement("th", "id");
    var th2 = createNewElement("th", "name");
    var th3 = createNewElement("th", "email");
    tr.append(th1, th2, th3);
    thead.append(tr);
    console.log(typeof(data))
    console.log(data)
    var tbody=document.createElement("tbody");
    var dataItems=data.slice(start,end);

    dataItems.map((element) => {
        var tr1 = document.createElement("tr");
        var td1 = createNewElement("td", element.id);
        var td2 = createNewElement("td", element.name);
        var td3 = createNewElement("td", element.email);
...
function showPagination(data) {
    ...
    for (var pageNo = 1; pageNo <= noOfPages; pageNo++) {
      pageList = pageList + `<li class="page-item"><button type="button" class="btn btn-link " onclick="getDataByPageNo(${pageNo})">${pageNo}</button></li>`
    }
    var previousStatus = ""
    if (currentPageId == 1) {
    ...
    paginationDiv.innerHTML = `
            <nav aria-label="Page navigation example">
              <ul class="pagination justify-content-center">
                <li class="page-item "><button type="button" class="btn btn-link ${previousStatus}" onclick="getDataByPageNo(${currentPageId - 1})">Previous</button></li>
                ${pageList}
                <li class="page-item "><button type="button" class="btn btn-link ${nextStatus}" onclick="getDataByPageNo(${currentPageId + 1})">Next</button></li>
              </ul>
            </nav>
    `
  }
